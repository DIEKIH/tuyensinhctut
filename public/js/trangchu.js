            ...
            <div class="news-item">
                    ...
                    <div class="news-content">
                        <div class="news-title one-line ellipsis-text pb-2">
                            <h3><a href="" class="text-hover">${tintucnoibat.tieude}</a></h3>
                        </div>
                    ...
                    <div class="news-info d-flex justify-content-start gap-3 text-sub">
                        <div class="news-date me-3">
                            <i class="fa-regular fa-clock me-1"></i> ${formatDate(tintucnoibat.ngaydang)}
                        </div>
                        <div class="news-views">
                            <i class="fa-regular fa-eye me-1"></i> ${tintucnoibat.views}
                        </div>
                    </div>
...
//     <div class="d-flex flex-column flex-md-row" style="align-items: stretch;">

//         <!-- Hình ảnh -->
//             <div class=" hover01 me-md-3 mb-3 mb-md-0 col-12 col-md-4 text-center d-flex flex-column" style="min-height: 150px;">
//                 <a href="">
//                     <figure>
...
//             <div class="mb-2 fs-4 lh-sm fw-semibold ellipsis-text one-line">
//                 <a href="/su-kien/${sukien.id}" class="text-hover">
//                     ${sukien.title}
//                 </a>
//             </div>

//             <div class="ellipsis-text two-line mt-2 mb-1">
//                 ${sukien.summary}
//             </div>
//             <div class="ellipsis-text two-line mt-2 mb-1">
//                 <i class="fa-regular fa-clock"></i> Ngày diễn ra: ${formattedDate}
//             </div>
//             <div class="ellipsis-text mt-1 two-line">
...
function renderSukiennho(sukiennhoArray) {
    ...
    sukiennhoArray.forEach((sukien, index) => {
        ...
        const formattedDate = formatDate(sukien.thoigian_bd);

        const html = `
            <div class="col-md-6 col-lg-6">
                <div class="position-relative ${isLast ? 'fake-news-card' : ''}">
                    ${isLast ? '<div class="fake-news-blur d-flex flex-column flex-md-row w-100" style="align-items: stretch;">' :
                '<div class="d-flex flex-column flex-md-row" style="align-items: stretch;">'}
                    
                        <!-- Hình ảnh -->
                        <div class="hover01 me-md-3 mb-3 mb-md-0 col-12 col-md-4 text-center d-flex flex-column" style="min-height: 150px;">
                            <a href="/su-kien/${sukien.id}">
                                <figure>
                                    <img src="${sukien.image_url}" alt="Hình sự kiện"
                                        style="width: 100%; height: 100%; object-fit: cover;">
                                </figure>
                            </a>
                        </div>
                        ...
                        <div class="flex-grow-1 d-flex flex-column">
                            ...
                            <div class="ellipsis-text one-line mt-2 mb-1">
                                ${sukien.tomtat}
                            </div>
                            <div class="ellipsis-text two-line mt-2 mb-1">
                                <i class="fa-regular fa-clock"></i> Ngày diễn ra: ${formattedDate}
                    ...
                    </div> <!-- đóng div d-flex / fake-news-blur -->

                    ${isLast ? `
                        <!-- Overlay khi hover màn hình lớn -->
                        <a href="/su-kien/${sukien.id}" class="position-absolute top-0 start-0 w-100 h-100 d-none d-lg-flex align-items-center justify-content-center fake-news-overlay text-decoration-none">
                            <div class="d-inline-block button">
                               Sự kiện khác
                                <svg fill="currentColor" viewBox="0 0 24 24" class="icon">
                        ...
                        <!-- Overlay luôn hiển thị trên mobile -->
                        <a href="/su-kien/${sukien.id}" class="position-absolute top-0 start-0 w-100 h-100 d-flex d-lg-none align-items-center justify-content-center fake-news-overlay text-decoration-none">
                            <div class="d-inline-block button">
                                Sự kiện khác
                                <svg fill="currentColor" viewBox="0 0 24 24" class="icon">
                                    <path clip-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm4.28 10.28a.75.75 0 000-1.06l-3-3a.75.75 0 10-1.06 1.06l1.72 1.72H8.25a.75.75 0 000 1.5h5.69l-1.72 1.72a.75.75 0 101.06 1.06l3-3z" fill-rule="evenodd"></path>
...
//     const newBadge = item.new === 1 ? `<img src="/images/new.gif" alt="New" class="me-1">` : '';
//     return `
...
function createThongbaonho(item, offcanvasId, isLast = false) {
    if (isLast) {
        return `
            <li class="ellipsis-text two-line mb-4 noti-item fake-news-card">
                <!-- Nội dung thông báo (bị làm mờ) -->
                <div class="fake-news-blur">
                    ${item.tieude}
                </div>

                <!-- Overlay chữ "Xem thêm" (chỉ hiện khi hover) -->
                <a href="/thong-bao"
                   class="fake-news-overlay text-decoration-none w-100 h-100 position-absolute top-0 start-0">
                    <span class="fw-bold px-3 py-1 rounded">Xem thêm >></span>
                </a>
            </li>
        `;
    }
...
function renderThongbaonho(data) {
    ...
    data.forEach(function (item, index) {
        const offcanvasId = `offcanvas${item.id}`;
        const pdfContainerId = `${offcanvasId}-pdf`;
        const isLast = index === data.length - 1; // check phần tử cuối

        const itemHtml = createThongbaonho(item, offcanvasId, isLast);
        const offcanvasHtml = createOffcanvas(offcanvasId, item.tieude);
